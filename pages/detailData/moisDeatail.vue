...
// 请将此替换为实际的后端接口地址
const apiUrl = 'http://192.168.20.237:8080/getMoisTop10';
...
onMounted(async () => {
  ...
  try {
    ...
    const seriesData = processedData.map(item => item.soilMoisture);
	xAxisData.reverse();
	seriesData.reverse();
    const option = {
      title: { text: '详细土壤湿度信息' },
      tooltip: {
        ...
        formatter: (params) => `${params.name}<br>土壤湿度: ${params.value}%`
      },
      legend: { data: ['土壤湿度'] },
      grid: {
        left: '3%',
    ...
    echartInstance.on('click', (params) => {
      console.log('点击时间:', params.name, '土壤湿度:', params.value);
    });

    echartInstance.on('longpress', (params) => {
      console.log('长按时间:', params.name, '土壤湿度:', params.value);
    });
  } catch (error) {
